...
class GATConv2(MessagePassing):
  ...
  #  print("jeeej")
    ...
    def message_and_aggregate(self, adj_t, x: Tensor, alpha: Tensor) -> Tensor:
        print("CSR")
        
        # Sprawdzenie typu x
        if isinstance(x, tuple):
            print("x is a tuple, selecting first element")
            x = x[0]  # Wybierz pierwszy tensor, jeśli x jest krotką

        # Wydobycie danych CSR
        indices = adj_t.storage.col()
        indptr = adj_t.storage.rowptr()
        ...
        x_j = spmm_csr(indices, indptr, data, x)

        print("CSR2")  # Assuming your custom spmm can handle CSR
        weighted_messages = alpha.unsqueeze(-1) * x_j
        print("CSR3")  # Weight messages with attention coefficients
